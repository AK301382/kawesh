...
const PortfolioPage = () => {
  ...
  const stats = [
    { number: "200+", label: t('portfolio:stats.projectsDelivered'), icon: Rocket },
    { number: "150+", label: t('portfolio:stats.happyClients'), icon: Award },
    { number: "98%", label: t('portfolio:stats.successRate'), icon: TrendingUp },
    { number: "18", label: t('portfolio:stats.workSamples'), icon: Star }
  ];
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta name="keywords" content={t('portfolio:seo.keywords')} />
        
        {/* Open Graph */}
        <meta property="og:title" content={t('portfolio:seo.ogTitle')} />
        <meta property="og:description" content={t('portfolio:seo.ogDescription')} />
        ...
        <meta name="twitter:title" content={t('portfolio:seo.ogTitle')} />
        <meta name="twitter:description" content={t('portfolio:seo.ogDescription')} />
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <h1 className="text-5xl md:text-6xl font-bold mb-6">
                <span className="text-gray-900 dark:text-white">{t('portfolio:hero.title1')}</span>
                <br />
              ...
              <div className="flex flex-wrap justify-center gap-6 mb-8">
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                  ...
                  <span className="font-medium">{t('portfolio:hero.highlights.awardWinning')}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                  <CheckCircle2 className="w-5 h-5 text-green-600 dark:text-green-400" />
                  <span className="font-medium">{t('portfolio:hero.highlights.provenResults')}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                  <CheckCircle2 className="w-5 h-5 text-green-600 dark:text-green-400" />
                  <span className="font-medium">{t('portfolio:hero.highlights.clientSuccess')}</span>
                </div>
              </div>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            {filteredProjects.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-xl text-gray-600 dark:text-gray-400">{t('portfolio:noProjects')}</p>
              </div>
            ) : (
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-wrap justify-center gap-6 mb-10">
                <div className="flex items-center gap-2 text-white">
                  ...
                  <span>{t('buildSite:cta.benefits.freeConsultation')}</span>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <CheckCircle2 className="w-5 h-5" />
                  <span>{t('buildSite:cta.benefits.customProposal')}</span>
                </div>
                <div className="flex items-center gap-2 text-white">
                  <CheckCircle2 className="w-5 h-5" />
                  <span>{t('buildSite:cta.benefits.provenProcess')}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/contact">
                  <Button size="lg" className="bg-white text-blue-600 hover:bg-gray-100 shadow-xl hover:shadow-2xl transition-all" data-testid="cta-start-project">
                    {t('buildSite:buttons.startProject')}
                    <ArrowRight className="ml-2 w-5 h-5" />
                  </Button>
                </Link>
                <Link to="/services">
                  <Button size="lg" variant="outline" className="border-2 border-white text-white hover:bg-white/10" data-testid="cta-explore-services">
                    {t('buildSite:buttons.exploreServices')}
                  </Button>
                </Link>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-white/20">
                <p className="text-blue-100 text-sm mb-4">{t('portfolio:trustIndicators.trustedBy')}</p>
                <div className="flex justify-center items-center gap-8 flex-wrap opacity-80">
                  <div className="text-white font-semibold">{t('portfolio:trustIndicators.projectsCount')}</div>
                  <div className="w-1 h-1 bg-white rounded-full"></div>
                  <div className="text-white font-semibold">{t('portfolio:trustIndicators.successRate')}</div>
                  <div className="w-1 h-1 bg-white rounded-full"></div>
                  <div className="text-white font-semibold">{t('portfolio:trustIndicators.awardWinning')}</div>
                </div>
              </div>
            </div>
          </div>
