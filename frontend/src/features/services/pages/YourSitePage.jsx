...
const YourSitePage = () => {
  const { t } = useTranslation(['buildSite']);
  ...
  // {t('buildSite:sections.premiumTemplates.title')} - 20 businesses with special features (now using local images)
      ...
      <div className="p-6">
        ...
        {isPremium && template.features && (
          <div className="mb-4 space-y-2">
            <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('buildSite:features.specialFeatures')}</p>
        ...
        >
          ...
          {t('buildSite:buttons.viewTemplate')}
        </Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-gray-800 dark:to-gray-700 rounded-2xl p-8 border-2 border-blue-200 dark:border-cyan-600">
                <p className="text-lg md:text-xl text-gray-800 dark:text-gray-200 font-medium leading-relaxed">
                  ...
                  <strong className="text-blue-600 dark:text-cyan-400">{t('buildSite:hero.highlight')}</strong>
                  <span className="text-2xl ml-2">🚀</span>
                </p>
              </div>
              
              <p className="text-lg text-gray-600 dark:text-gray-400">
                {t('buildSite:hero.description2')}
              </p>
              
              <div className="flex items-center justify-center gap-4 text-lg font-bold text-green-600 dark:text-green-400">
                <Zap className="w-6 h-6" />
                <span>{t('buildSite:hero.freeToStart')}</span>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="relative w-full md:w-2/3 lg:w-1/2">
              ...
              <input
                ...
                placeholder={t('buildSite:search.placeholder')}
                value={searchQuery}
          ...
          {searchQuery && (
            <div className="mt-4 text-center">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('buildSite:search.found')} <span className="font-bold text-blue-600 dark:text-cyan-400">{allTemplates.length}</span> {t('buildSite:search.templatesMatching')} "{searchQuery}"
              </p>
            </div>
          )}
        </div>
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-16">
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 text-blue-600 dark:text-cyan-400 px-4 py-2 rounded-full text-sm font-semibold mb-4">
                <Sparkles className="w-4 h-4" />
                {t('buildSite:sections.allTemplates.badge')}
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4">
                {t('buildSite:sections.allTemplates.title')}
              </h2>
              <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
                {t('buildSite:sections.allTemplates.description')}
              </p>
            </div>
            ...
            {allTemplates.length > 0 ? (
                ...
                {visibleAllTemplates.length < allTemplates.length && (
                  <div className="text-center mt-12">
                    ...
                    >
                      ...
                      {t('buildSite:buttons.loadMore')} ({allTemplates.length - visibleAllTemplates.length} {t('buildSite:messages.remaining')})
                    </Button>
                  </div>
                )}
              </>
              ...
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('buildSite:messages.noTemplatesFound')}</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">{t('buildSite:messages.noTemplatesDescription')}</p>
                <Button
                  onClick={() => setSearchQuery('')}
                  className="bg-blue-600 hover:bg-blue-700 text-white"
                >
                  {t('buildSite:buttons.clearSearch')}
                </Button>
              </div>
            )}
          </div>
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-16">
              <div className="inline-flex items-center gap-2 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 px-4 py-2 rounded-full text-sm font-semibold mb-4">
                <Sparkles className="w-4 h-4" />
                {t('buildSite:sections.freeTemplates.badge')}
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4">
                {t('buildSite:sections.freeTemplates.title')}
              </h2>
              <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
                {t('buildSite:sections.freeTemplates.description')}
              </p>
            </div>
            ...
            {filteredFreeTemplates.length > 0 ? (
              <>
                ...
                {freeDisplayCount < filteredFreeTemplates.length && (
                  <div className="text-center mt-12">
                    ...
                    >
                      <ChevronDown className="w-5 h-5" />
                      {t('buildSite:messages.loadMoreFree')} ({filteredFreeTemplates.length - freeDisplayCount} {t('buildSite:messages.remaining')})
                    </Button>
                  </div>
                )}
              </>
              ...
              <div className="text-center py-20">
                <Search className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('buildSite:messages.noFreeTemplatesFound')}</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">{t('buildSite:messages.noTemplatesDescription')}</p>
                <Button
                  onClick={() => setSearchQuery('')}
                  className="bg-blue-600 hover:bg-blue-700 text-white"
      ...
      {/* {t('buildSite:sections.premiumTemplates.title')} Section */}
      {filterType === 'premium' && (
          ...
          <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-16">
              <div className="inline-flex items-center gap-2 bg-gradient-to-r from-yellow-100 to-orange-100 dark:from-yellow-900/30 dark:to-orange-900/30 text-orange-600 dark:text-orange-400 px-4 py-2 rounded-full text-sm font-semibold mb-4">
                <Crown className="w-4 h-4" />
                {t('buildSite:sections.premiumTemplates.badge')}
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-4">
                {t('buildSite:sections.premiumTemplates.title')}
              </h2>
              <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
                {t('buildSite:sections.premiumTemplates.description')}
              </p>
            </div>
            ...
            {filteredPremiumTemplates.length > 0 ? (
              <>
                ...
                {premiumDisplayCount < filteredPremiumTemplates.length && (
                  <div className="text-center mt-12">
                    ...
                    >
                      <ChevronDown className="w-5 h-5" />
                      {t('buildSite:messages.loadMorePremiumCount')} ({filteredPremiumTemplates.length - premiumDisplayCount} {t('buildSite:messages.remaining')})
                    </Button>
                  </div>
                )}
              </>
              ...
              <div className="text-center py-20">
                <Search className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('buildSite:messages.noPremiumTemplatesFound')}</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-6">{t('buildSite:messages.noTemplatesDescription')}</p>
                <Button
                  onClick={() => setSearchQuery('')}
                  className="bg-blue-600 hover:bg-blue-700 text-white"
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-blue-600 to-cyan-500 dark:from-blue-800 dark:to-cyan-700">
        <div className="container mx-auto max-w-4xl text-center">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">
            {t('buildSite:cta.title')}
          </h2>
          <p className="text-xl text-blue-100 mb-8">
            {t('buildSite:cta.description')}
          </p>
          <Button 
          ...
          >
            {t('buildSite:buttons.getStarted')}
          </Button>
        </div>
      </section>
    </div>
