...
async def get_current_customer(
    ...
    try:
        ...
        if not customer.is_active:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=translate("errors.auth.customer_deactivated", locale),
                headers={"WWW-Authenticate": "Bearer"},
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=translate("errors.auth.could_not_validate", locale),
            headers={"WWW-Authenticate": "Bearer"},
        )
...
async def get_current_account(
    ...
    try:
        ...
        if not account:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=translate("errors.auth.account_not_found", locale),
                headers={"WWW-Authenticate": "Bearer"},
            )
